...
static void _applyJson(const char* line, TamaState* out) {
  ...
  if (!t.isNull() && t.size() == 2) {
    // M5StickS3 has no hardware RTC — M5.Rtc.setDate/setTime are no-ops
    // on this device, and reading back returns garbage. Maintain our own
    // epoch reference and derive time from millis() elapsed.
    extern time_t _epochBase;
    extern uint32_t _epochMs;
    extern bool _epochValid;
    extern uint32_t _clkLastRead;
    _epochBase = (time_t)t[0].as<uint32_t>() + (int32_t)t[1];
    _epochMs   = millis();
    _epochValid = true;
    _clkLastRead = 0;
    _rtcValid = true;
