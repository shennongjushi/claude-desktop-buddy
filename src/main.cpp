...
// M5StickS3: LED controlled via M5.Power.setLed() instead of GPIO
...
bool    menuOpen    = false;
uint8_t menuSel     = 0;
uint8_t brightLevel = 4;           // 0..4 → setBrightness 40..240
bool    btnALong    = false;
...
static void applyBrightness() { M5.Display.setBrightness(40 + brightLevel * 50); }
...
bool                   _onUsb       = false;   // non-static: xfer.h reads this for status JSON
// M5StickS3 has no hardware RTC. Desktop pushes epoch via {"time":[...]}
// which data.h stores in _epochBase + _epochMs; we derive clock from
// millis() elapsed since that reference.
time_t   _epochBase  = 0;
uint32_t _epochMs    = 0;
bool     _epochValid = false;
static void clockRefreshRtc() {
  if (millis() - _clkLastRead < 1000) return;
  _clkLastRead = millis();
  // At 100% battery, S3's charger does trickle cycling — isCharging()
  // flips on/off every few seconds even with USB steadily connected.
  // Asymmetric debounce: set true immediately on any charging signal,
  // only drop to false after 30 consecutive non-charging seconds.
  bool raw = M5.Power.isCharging() != m5::Power_Class::is_discharging;
  static int8_t offStreak = 0;
  if (raw) { _onUsb = true; offStreak = 0; }
  else if (_onUsb && ++offStreak >= 30) { _onUsb = false; offStreak = 0; }
  if (_epochValid) {
    time_t now = _epochBase + (time_t)((millis() - _epochMs) / 1000);
...
static uint8_t clockDow() {
  // weekDay is int8_t — guard against garbage RTC values (S3 has no RTC
  // chip, so reads can be uninitialized). C's signed modulo can go
  // negative and index DOW[] out of bounds.
  int w = _clkDt.weekDay;
  if (w < 0 || w > 6) return 0;
  return (uint8_t)w;
}
...
void drawInfo() {
  ...
  if (infoPage == 0) {
    ...
    bool usb = chgState != m5::Power_Class::is_discharging;
    bool charging = usb && iBat_mA > 1;
    bool full = usb && pct >= 99;
...
void setup() {
  auto cfg = M5.config();
  M5.begin(cfg);
  Serial.begin(115200);   // ESP32-S3 native USB CDC needs explicit init
  M5.Display.setRotation(0);
  M5.Speaker.begin();
  startBt();
...
void loop() {
  ...
  if (activeState == P_ATTENTION && settings().led) {
    M5.Power.setLed((now / 400) % 2 ? 128 : 0);
  } else {
    M5.Power.setLed(0);
  }
